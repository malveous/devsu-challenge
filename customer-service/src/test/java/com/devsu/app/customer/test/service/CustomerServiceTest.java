...
class CustomerServiceTest {
    ...
    void list_all_customers_when_success() {
        var anotherCustomer = Customer.builder().id(1).name("Charlie").address("House next to redhead girl")
                .phoneNumber("98789981").password("SnoopyBF").status(Boolean.TRUE).gender("Male").build();

        Mockito.when(customerRepository.findAll())
                .thenReturn(List.of(this.buildCustomerDataCompleted(), anotherCustomer));
    ...
    void find_customer_by_id_when_no_entry_found() {
        ...
        CustomerNotFoundException thrown = assertThrows(CustomerNotFoundException.class,
                () -> this.customerService.findCustomerById(100));
    ...
    void find_customer_by_personal_id_when_success() {
        var testCustomer = this.buildCustomerDataCompleted();
        Mockito.when(customerRepository.findByPersonalId(testCustomer.getPersonalId()))
    ...
    void find_customer_by_personal_id_when_no_entry_found() {
        ...
        CustomerNotFoundException thrown = assertThrows(CustomerNotFoundException.class,
                () -> this.customerService.findCustomerByPersonalId("100"));
    ...
    void find_customer_by_customer_id_when_success() {
        var testCustomer = this.buildCustomerDataCompleted();
        Mockito.when(customerRepository.findByCustomerId(testCustomer.getCustomerId()))
                .thenReturn(Optional.of(testCustomer));
    ...
    void find_customer_by_customer_id_when_no_entry_found() {
        ...
        CustomerNotFoundException thrown = assertThrows(CustomerNotFoundException.class,
                () -> this.customerService.findCustomerByCustomerId("1111-2222-3333-4444"));
    ...
    void register_customer_when_success() {
        ...
        Mockito.when(customerRepository
                .save(ArgumentMatchers.argThat(c -> c.getPersonalId().equals(testCustomer.getPersonalId()))))
                .thenReturn(testCustomer);
        var newCustomerDto = this.buildCustomerDtoDataCompleted();

        var newCustomerCode = this.customerService.register(newCustomerDto);
    ...
    void update_customer_when_success() {
        var testCustomer = this.buildCustomerDataCompleted();
        var testCustomerDto = this.buildCustomerDtoDataCompleted();
        Mockito.when(this.customerRepository.findCustomerBySpecificIdsIfExist(testCustomer.getCustomerId(),
                testCustomer.getPersonalId())).thenReturn(Optional.of(testCustomer));
        Mockito.when(customerRepository.save(testCustomer)).thenReturn(testCustomer);

        var newCustomerCode = this.customerService.update(testCustomerDto);

        Assertions.assertThat(newCustomerCode).isPositive();
    }

    private Customer buildCustomerDataCompleted() {
        return Customer.builder().id(1).name("Snoopy").gender("Male").age(18).personalId("12345678")
                .customerId("0000-1111-2222-3333-4444").address("House outside backyard").phoneNumber("98978172")
                .password("CharlieBF").status(Boolean.TRUE).build();
    }

    private CustomerDto buildCustomerDtoDataCompleted() {
        return new CustomerDto(this.buildCustomerDataCompleted());
    }
}
